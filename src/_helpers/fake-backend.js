    ...
    return new Promise((resolve, reject) => {
      ...
      function authenticate() {
        ...
        const user = users.find(x => x.email === email &&
                                     x.password === password && x.isVerified);
        if (!user)
      ...
      function register() {
          ...
          setTimeout(() => {
            alertService.info(
                `
                            ...
                            <p>Your email ${
                    user.email} is already registered.</p>
        ...
        setTimeout(() => {
          const verifyUrl =
              `${window.location.origin}/account/verify-email?token=${
      ...
      function verifyEmail() {
        const {token} = body();
        const user = users.find(x => !!x.verificationToken &&
      ...
      function forgotPassword() {
        ...
        setTimeout(() => {
          const resetUrl =
              `${window.location.origin}/account/reset-password?token=${
                  user.resetToken}`;
          alertService.info(`
      ...
      function validateResetToken() {
        const {token} = body();
        const user = users.find(x => !!x.resetToken && x.resetToken === token &&
      ...
      function isAuthenticated() {
        return (opts.headers['Authorization'] || '')
            .startsWith('Bearer fake-jwt-token');
      }

      function isAuthorized(role) {
        return isAuthenticated() &&
               opts.headers['Authorization'].split('.')[1] === role;
      }
      ...
      function body() { return opts.body && JSON.parse(opts.body); }
